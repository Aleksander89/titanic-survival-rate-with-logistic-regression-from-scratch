...
class LogisticRegression():
    """
    This implementation minimizes the objective function with batch gradient descent.
    """
    ...
    def fit(self, X, Y, num_iterations=50000, learning_rate=0.001, lmbda=0, plot_cost=True):
        _, params, _ = gradient_descent(X, Y, num_iterations, learning_rate, lmbda, plot_cost)
        self._coeff = params

    def predict(self, X, decision_boundary=0.5):
        X = np.vstack([np.ones(X.shape[0]), X.T])
        y_hat = sigmoid(np.dot(X.T, self._coeff))
...
def gradient_descent(X, Y, num_iterations, learning_rate, lmbda, plot_cost):
        X = np.vstack([np.ones(X.shape[0]), X.T])           # 
